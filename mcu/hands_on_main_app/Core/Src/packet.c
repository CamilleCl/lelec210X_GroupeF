...
void tag_cbc_mac(uint8_t *tag, const uint8_t *msg, size_t msg_len) {
    ...
    for (int i = 0; i < ((int) (msg_len/16)); i++){
    	AES128_encrypt(msg+16*i, state);
    }

    int rest = msg_len % 16;
    if(rest) {
    	for(i = 0; i < rest; i++) {
    		last_block[i] = (msg+msg_len/16)[i];
    	}
    	AES128_encrypt(last_block, state);
    }
...
//	memset(packet + payload_len + PACKET_HEADER_LENGTH, 0, PACKET_TAG_LENGTH);
    ...
    packet[0] = 0x00;
    packet[1] = sender_id;
    packet[2] = (uint8_t) payload_len >> 8;
    packet[3] = (uint8_t) payload_len;
    packet[4] = (uint8_t) serial >> 24;
    packet[5] = (uint8_t) serial >> 16;
    packet[6] = (uint8_t) serial >> 8;
    packet[7] = (uint8_t) serial;
